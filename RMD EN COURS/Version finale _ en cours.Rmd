...
Le processeur (ou CPU, pour Central Processing Unit) la partie qui exécute les instructions des programmes et effectue les calculs nécessaires au bon fonctionnement de l'ordinateur. Il peut être vu comme "l'organe" qui contrôle l'exécution des tâches. Un processeur peut contenir un ou plusieurs cœurs (ou cores), chacun capable de traiter des instructions indépendamment.
...
foreach permet d’écrire des boucles parallèles, tandis que doParallel les connecte à un cluster.
...
print(resultats)
``` 
Ici, puisqu'on a définit 4 workers et qu'il y a 5 calculs à faire, c'est le 1er coeur à finir qui va se charger du dernier calcul. 


## 3. Package **future + future.apply** 
